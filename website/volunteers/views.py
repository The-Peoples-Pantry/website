...
class ChefSignupListView(
    LoginRequiredMixin, GroupRequiredMixin, LastVisitedMixin, FilterView
):
    ...
    def get_queryset(self):
        return (
            super()
            .get_queryset()
            .with_delivery_distance(chef=self.request.user)
            .order_by("delivery_distance")
        )
    ...
    def extra_context(self):
        return {
            "forms": [
                ChefSignupForm(instance=meal_request)
...
class DelivererSignupListView(
    ...
    queryset = (
        MealRequest.objects.available_for_deliverer_signup().with_delivery_distance()
    )
    ...
    def extra_context(self):
        return {
            "forms": [
                DelivererSignupForm(instance=meal_request)
                for meal_request in self.object_list
            ],
        }
...
class ChefTaskListView(LoginRequiredMixin, GroupRequiredMixin, ListView):
    ...
    def extra_context(self):
        return {
            "delivered": {
                "forms": [
                    ChefTaskForm(instance=meal_request)
                    for meal_request in self.object_list.delivered()
                ],
            },
            "not_delivered": {
                "forms": [
                    ChefTaskForm(instance=meal_request)
                    for meal_request in self.object_list.not_delivered()
                ],
            },
        }
    ...
    def post(self, request):
        ...
        if meal_request.delivery_date <= date.today():
            ...
            messages.success(
                ...
                % (
                    meal_request.pk,
                    meal_request.address,
                    settings.VOLUNTEER_COORDINATORS_EMAIL,
                ),
            )
        else:
            messages.error(
...
class VolunteerApplicationView(FormView, UpdateView):
    ...
    def form_valid(self, form):
        ...
        application = VolunteerApplication.objects.create(
            user=self.request.user, role=self.role, organizer_teams=organizer_teams
        )
