...
def time_difference(t_start, t_end):
    ...
    return datetime.datetime.combine(today, t_end) - datetime.datetime.combine(
        today, t_start
    )
...
class VolunteerApplicationForm(forms.ModelForm):
    ...
    accept_terms = forms.BooleanField(
        label="I have carefully read and understood these terms.", required=True
    )
...
class ChefSignupForm(forms.ModelForm):
    can_deliver = forms.BooleanField(
        label="I can also deliver this meal myself", required=False
    )
    ...
    def clean(self):
        ...
        if pickup_end <= pickup_start:
            self.add_error(
                "pickup_end",
                ValidationError(
                    "The pickup end time must come after the pickup start time"
                ),
            )
        ...
        if can_deliver:
            if dropoff_end <= dropoff_start:
                self.add_error(
                    "dropoff_end",
                    ValidationError(
                        "The dropoff end time must come after the dropoff start time"
                    ),
                )
            if dropoff_start < pickup_start:
                self.add_error(
                    "dropoff_start",
                    ValidationError(
                        "The dropoff start time cannot come before the pickup start time"
                    ),
                )
            if time_difference(dropoff_start, dropoff_end) > datetime.timedelta(
                hours=2
            ):
                self.add_error(
                    "dropoff_end",
                    ValidationError(
                        "The delivery window cannot be longer than 2 hours"
                    ),
                )
...
class DelivererSignupForm(forms.ModelForm):
    ...
    def clean(self):
        ...
        if time_difference(dropoff_start, dropoff_end) > datetime.timedelta(hours=2):
            self.add_error(
                "dropoff_end",
                ValidationError("The delivery window cannot be longer than 2 hours"),
            )
